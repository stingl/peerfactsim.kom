...
package de.tud.kom.p2psim.impl.network.mobile.MovementModel;

import org.apache.log4j.Logger;

import de.tud.kom.p2psim.impl.network.mobile.MobileNode;
...
public class RandomWaypoint implements MovementInstance  {
	...
	public void updateMovement(MobileNode m) {
		...
		double distance = Math.sqrt(Math.pow(xDest - m.getXPos(), 2) + Math.pow(yDest - m.getYPos(), 2));
		...
		if (traveledDistance > 0) {
			...
			double xPos1 = Math.cos(Math.atan2(yDest - m.getYPos(), xDest - m.getXPos())) * (timePassed * speed)  + m.getXPos();
			double yPos1 = Math.sin(Math.atan2(yDest - m.getYPos(), xDest - m.getXPos())) * (timePassed * speed)  + m.getYPos();
			...
			m.setYPos(yDest);
			inTransit = false;
			lastChanged = lastChanged + distance / speed; //Achtung.. �nderung..�berpr�fen!
			while (!inTransit)
