...
public class GeographicalPositioning implements PositioningStrategy {
	...
	static class GeographicalPosition implements NetPosition {
		...
		public double getDistance(NetPosition netPosition) {
			if (!(netPosition instanceof GeographicalPosition)) throw new AssertionError("Can not calculate the distance between two different position classes: "
					+ this.getClass() + " and " + netPosition.getClass());
			GeographicalPosition other = (GeographicalPosition)netPosition;

			return GeoToolkit.getDistance(this.latitude, this.longitude, other.latitude, other.longitude);
		}
