...
public class QuerySuccessAndNHops {
	...
	final QueryNHops qnhops = this.new QueryNHops();

	final AvgRespTime avgRespTime = this.new AvgRespTime();

	Queue<QueryHandle> queries = new LinkedBlockingQueue<QueryHandle>();

	Map<Object, QueryHandle> queriesMap = new HashMap<Object, QueryHandle>();
		...
		try {
			while (queries.element().getTimeStarted() <= time - TIMEOUT) {
				...
				if (CONSIDER_FAILED_FOR_HOPS) {
					...
					// if (hopCount > 0)
					// System.out.println("Considered >0 Hop Count: " + hopCount
					// + "for failed query.");
				}
	...
	public class QueryNHops extends AverageMetric {
	...
	public class AvgRespTime extends AverageMetric {
		AvgRespTime() {
			// Protected
		}
