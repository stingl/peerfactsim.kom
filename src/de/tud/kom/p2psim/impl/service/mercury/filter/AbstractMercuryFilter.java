...
public abstract class AbstractMercuryFilter implements IMercuryFilter {
	...
	public AbstractMercuryFilter(String name, AttributeType type, OPERATOR_TYPE operator) {
		this.name = name;
		this.type = type;
	...
	public boolean match(IMercuryAttribute attribute) {
		...
		if (!attribute.getName().equals(this.getName()) || !attribute.getType().equals(this.getType())) {
			// Only match, if Attribute and Type are the same - should be implemented by MercurySubscription
			return true;
		}
	...
	private void operatorNotDefined() {
		throw new RuntimeException("Operator " + this.getOperator()
				+ " is not defined for AttributeType " + this.getType().toString());
	}
