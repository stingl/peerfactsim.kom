...
import de.tud.kom.p2psim.impl.skynet.AbstractUpdateStrategy;
...
public class AttributeUpdateStrategy extends
	...
	public void setDataToSend() {
		...
		if (skyNetNode.getTreeHandler().isRoot()) {
			/*
			 * HashMap<BigDecimal, AttributeSubCoordinatorInfo> subCoList =
			...
			int defSend = 0;
			int sendMax = ((SkyNetHostProperties) skyNetNode.getHost()
			...
			// parentCoordinator
			if (definedAmount) {
				if (Math.min(sendMax, actualAttributeEntries) > maxAttributeEntriesForCo) {
					defSend = maxAttributeEntriesForCo - 1;
				} else {
					defSend = Math.min(sendMax, actualAttributeEntries) - 1;
				}
			} else {
			...
			int offset = 0;
			this.requestedAttributeEntriesToSend = Math.min(sendMax,
			...
			if (supportPeer != null
					&& Math.min(sendMax, actualAttributeEntries) > offset) {
				setAttributesToSendForSP(Math.min(sendMax,
						actualAttributeEntries)
						- (offset + 1), offset);
			}
		}
	...
	public void removeStaleSubCoordinatorsOLD() {
		...
		boolean cycle = true;
		long actualTime = Simulator.getCurrentTime();
		HashMap<BigDecimal, AttributeSubCoordinatorInfo> list = attributeStorage
	...
	private boolean checkObject(SkyNetNodeInfo parentCo) {
		if (parentCo != null) {
