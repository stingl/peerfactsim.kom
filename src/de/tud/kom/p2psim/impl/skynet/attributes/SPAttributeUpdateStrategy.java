...
import de.tud.kom.p2psim.impl.skynet.SkyNetNodeInfoImpl;
...
public class SPAttributeUpdateStrategy extends
	...
	public void setDataToSend() {
		if (brotherCoordinator.getLevel() == 0) {
			...
			int defSend = 0;
			int sendMax = ((SkyNetHostProperties) supportPeer.getHost()
			...
			if (definedAmount) {
				if (Math.min(sendMax, actualAttributeEntries) > maxAttributeEntriesForCo) {
					...
					defSend = Math.min(sendMax, actualAttributeEntries);
				}
			} else {
	...
	public boolean checkObject(SkyNetNodeInfo parentCo) {
		if (parentCo != null) {
