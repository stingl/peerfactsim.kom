...
public class SkyNetBatchSimulator implements Configurable {
	...
	private void writeMainGnuPlotFile(File dir, BufferedWriter bw) {
		try {
	...
	private String createSimulationDirPostfix() {
		...
		ret += "_" + instance.getIntProperty("SkyNetTreeBranchingFactor")
				+ "TBF";

		// adding the information of the metrics-update-settings
		ret += "_" + instance.getStringProperty("MetricUpdateTime") + "MUT";
		ret += instance.getIntProperty("TimeForGeneratingSystemStatistics")
				+ "TGSS";
		if (instance.getBooleanProperty("AlwaysPushSystemStatistics")) {
			ret += "1APSS";
		} else {
			ret += "0APSS";
		}
		ret += instance.getStringProperty("MetricRemoveStaleSubCo").replace(
				".", "")
				+ "MRSS";

		// adding the information of the polishing-mode-settings
		String polishingMode = instance.getStringProperty("SmoothingType");
		if (polishingMode.equals("MedianSmoothing")) {
			ret += "_MedPol";
			ret += instance.getIntProperty("SizeOfHistory") + "H";
		} else {
			ret += "_ExpPol";
			ret += instance.getIntProperty("SizeOfHistory") + "H";
			ret += instance.getStringProperty("ExponentialSmoothingFactor")
					.replace(".", "")
					+ "F";
		}

		// adding the information of the attribute-update-settings
		ret += "_" + instance.getStringProperty("AttributeUpdateTime") + "AUT";
		ret += instance.getIntProperty("LowerBoundOfEntriesForCo") + "LBE";
		ret += instance.getStringProperty("AttributeRemoveStaleSubCo").replace(
				".", "")
				+ "ARSS";
		return ret;
	}

	private void checkForSimDirectory() {
		File dir = new File(SkyNetConstants.COMMON_SIMULATIONS_PATH);
		if (dir.mkdir()) {
	...
	private void copyFile(String oldFile, File targetDir, String targetFile,
