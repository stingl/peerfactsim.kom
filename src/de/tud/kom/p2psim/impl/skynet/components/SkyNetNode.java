...
import de.tud.kom.p2psim.impl.skynet.analyzing.analyzers.QueryAnalyzer;
import de.tud.kom.p2psim.impl.skynet.attributes.AttributeEntry;
import de.tud.kom.p2psim.impl.skynet.attributes.operations.AttributeUpdateOperation;
import de.tud.kom.p2psim.impl.skynet.attributes.operations.ParentCoordinatorInformationOperation;
import de.tud.kom.p2psim.impl.skynet.metrics.MetricStorage;
...
import de.tud.kom.p2psim.impl.skynet.metrics.operations.AggregationResultDummyOperation;
import de.tud.kom.p2psim.impl.skynet.metrics.operations.AggregationResultMapDummyOperation;
import de.tud.kom.p2psim.impl.skynet.metrics.operations.MetricUpdateOperation;
import de.tud.kom.p2psim.impl.skynet.queries.Query;
import de.tud.kom.p2psim.impl.skynet.queries.QueryCreator;
import de.tud.kom.p2psim.impl.skynet.queries.QueryResolver;
...
public class SkyNetNode extends AbstractSkyNetNode implements
	...
	public SkyNetNode(SkyNetNodeInfo nodeInfo, short port,
			TransLayer transLayer, OverlayNode overlayNode, int simulationSize,
	...
	public int getAggregationResultMap(
		...
		if (((AbstractOverlayNode) getOverlayNode()).getPeerStatus().equals(
				PeerStatus.PRESENT)) {
			IAggregationMap<String> aggMap = getMetricsInterpretation()
	...
	public int getAggregationResult(final Object identifier,
		...
		if (((AbstractOverlayNode) getOverlayNode()).getPeerStatus().equals(
			...
			if (entry == null)
				return -2;
			IAggregationResult val = entry.getMetrics().get(identifier);
			if (val == null)
				throw new NoSuchValueException(identifier);
			AggregationResultDummyOperation op = new AggregationResultDummyOperation(
					this, new OperationCallback<IAggregationResult>() {
	...
	public void getAggregationResultAction(String identifier) {
		final long start = Simulator.getCurrentTime();
		int state = 0;
		try {
