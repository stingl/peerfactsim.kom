...
import de.tud.kom.p2psim.impl.skynet.analyzing.analyzers.postProcessing.StatusPostProcessor;
 ...
 * 
 * @author Dominik Stingl (extended by Julius Rueckert) <peerfact@kom.tu-darmstadt.de>
...
public class ChurnStatisticsAnalyzer extends AbstractSkyNetAnalyzer implements
	...
	private HashMap<Long, Host> offlinePeers;

	private static HashMap<Long, Host> onlinePeers;
	...
	public static void setCreatedHost(List<Host> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			host = list.get(i);
			if (host.getOverlay(NapsterServerNode.class) == null) {
	...
	public void setSimulationSize(int size) {
		...
		onlinePeers = new HashMap<Long, Host>((int) capacity);
		offlinePeers = new HashMap<Long, Host>((int) capacity);
	}
				...
				while (ipIter.hasNext()) {
					...
					if (node != null
						...
						if (secondDim == null) {
							...
							secondDim.put(new Integer(level), dataArr);
							idLevelMatrix.put(new Integer(periodeCounter),
									secondDim);
							...
							secondDim.put(new Integer(level), valueArr);
							yMax = Math.max(yMax, secondDim.lastKey()
									.intValue());
							idLevelMatrix.put(new Integer(periodeCounter),
									secondDim);
