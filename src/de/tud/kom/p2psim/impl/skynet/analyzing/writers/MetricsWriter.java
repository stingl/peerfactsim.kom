...
import de.tud.kom.p2psim.impl.skynet.visualization.SkyNetVisualization;
import de.tud.kom.p2psim.impl.skynet.visualization.util.DataSet;
import de.tud.kom.p2psim.impl.skynet.visualization.util.DeviationSet;
import de.tud.kom.p2psim.impl.skynet.visualization.util.VisualizationType;
...
public class MetricsWriter extends AbstractSkyNetWriter {
	...
	private static final String METRICS_FILE = "Metrics.dat";

	private static final String METRICS_PATH = "data";
	...
	private HashMap<String, MetricsAggregate> skyNetMetrics;

	private int skyNetNodeCounter;

	private HashMap<String, MetricsAggregate> aggMetrics;
	...
	private MetricsWriter() {
		initWriteDirectory(RESULTS_PATH + File.separator
				...
				+ File.separator + METRICS_PATH, false);
		...
		aggMetrics = new HashMap<String, MetricsAggregate>();
		ipTracer = new Vector<Long>();
		skyNetMetrics = new HashMap<String, MetricsAggregate>();
		skyNetNodeCounter = 0;
		// nodeCounter = 0;
		try {
			pw = new PrintWriter(new BufferedWriter(new FileWriter(RESULTS_PATH
					...
					+ METRICS_PATH + File.separator + METRICS_FILE)), true);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void addAggregatedMap(NetID id,
			HashMap<String, MetricsAggregate> map, boolean skyNet) {
		if (writeHeader) {
	...
	private boolean writeHeaderFromUnprocessedMap(
			HashMap<String, MetricsAggregate> aggMetrics) {
		if (aggMetrics.size() > 0) {
			pw.println("# Time in sec");
			pw.println("# Online-Peers");
			pw.println("# Offline-Peers");
			Iterator<String> nameIter = aggMetrics.keySet().iterator();
