...
public class PSense {
	 ...
	 */
	private boolean isAngleBetween(double angle, double startAngle,
			double endAngle) {
		// transform angle between 0 and 360 degree
		angle = modulo(angle, DEGREE_OF_CIRCLE);
		startAngle = modulo(startAngle, DEGREE_OF_CIRCLE);
		endAngle = modulo(endAngle, DEGREE_OF_CIRCLE);

		if (startAngle <= endAngle) {
			return startAngle <= angle && angle < endAngle;
		} else {
			return 0 <= angle && angle < endAngle && startAngle <= angle
					&& angle <= DEGREE_OF_CIRCLE;
		}
	}
	...
	private double computeAngle(int x, int y) {
		...
		double cosAngleRadian = Math.acos((double) x / length);
	 ...
	 */
	private boolean inVisionRange(Point point, Point point2,
