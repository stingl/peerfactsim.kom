...
import de.tud.kom.p2psim.impl.overlay.ido.von.messages.PeerMsg;
import de.tud.kom.p2psim.impl.overlay.ido.von.messages.SimpleAckMsg;
import de.tud.kom.p2psim.impl.overlay.ido.von.operations.HelloOperation;
...
public class VonMessageHandler implements TransMessageCallback,
	...
	private final HashMap<Integer, UnAckedMsg<InitialQueryMsg>> notAckedInitalQueries = new HashMap<Integer, UnAckedMsg<InitialQueryMsg>>();
	...
	public void messageTimeoutOccured(int commId) {
		...
		if (notAckedInitalQueries.containsKey(new Integer(commId))) {
			/*
		...
		if (msg instanceof ObtainIDMsg) {
			...
			if (node.isMaster && om.getSender().compareTo(VonID.EMPTY_ID) == 0) {
				...
				VonID newID = new VonID(
						node.lastAssignedID.getUniqueValue() + 1);
				node.lastAssignedID = newID;
	...
	private void forwardInitialQueryMsg(InitialQueryMsg qm,
		...
		if (log.isDebugEnabled())
			log.debug(node.getVonID()
					+ " forwarded InitialQueryMsg to "
					+ ((recInfo.getContact().getOverlayID() == null) ? recInfo
							.getContact().getTransInfo().getNetId() : recInfo
							.getContact().getOverlayID()) + " ("
					+ (numOfRetransmissions) + ". retransmission)");
	}

	private class UnAckedMsg<T extends Message> {
		public T msg;
		...
		public UnAckedMsg(T msg, VonNodeInfo recInfo) {
			this.msg = msg;
			this.recInfo = recInfo;
		}
