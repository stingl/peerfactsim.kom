...
public class Voronoi {
	...
	protected void insert(int id, Point coord) {
		...
		if (sites.containsKey(new Integer(id)) == false) {
			...
			sites.put(new Integer(id), new Point2d(coord.x, coord.y));
		}
	}
	...
	private void insert(VonNodeInfo nodeInfo) {
		Integer id = new Integer(nodeInfo.getContact().getOverlayID()
				.getUniqueValue());
	...
	public LinkedList<VonNodeInfo> insertAll(VonNodeInfo[] nodeInfo,
		...
		for (int i = 0; i < nodeInfo.length; i++) {
			...
			insert(nodeInfo[i], timestamps[i]);
		}
	...
	public synchronized VonNodeInfo remove(int id) {
		...
		VonNodeInfo nodeInfo = nodeInfos.remove(new Integer(id));
		timestamps.remove(new Integer(id));
	...
	public Point get(int id) {
		...
		Point2d p = sites.get(new Integer(id));
	...
	public boolean contains(int id, Point coord) {
		if (!sites.containsKey(new Integer(id)))
			return false;
	...
	private boolean intersectsBoundary(int idToCheck, Point center, int radius) {
		...
		Point2d coordOfNodeToCheck = sites.get(new Integer(idToCheck));
		...
		TreeMap<Point2d, Site> sites = getmSites();
		...
		if (sites.get(coordOfNodeToCheck) == null
				|| sites.get(coordOfNodeToCheck).edge_idxlist == null)
			return false;

		for (int edge_idx : sites.get(coordOfNodeToCheck).edge_idxlist) {
	...
	public boolean isAoiNeighbor(int centerNode, int toCheckNode, int aoiRadius) {
		...
		Point2d centerP = sites.get(new Integer(centerNode));
		Point2d toCheckP = sites.get(new Integer(toCheckNode));
	...
	private VonNodeInfo[] getVonNeighbors(int centerNode, int aoiRadius) {
		...
		Integer[] sites = getSitesArr();

		for (int i = 0; i < sites.length; i++) {
			if (isVonNeighbor(centerNode, sites[i], aoiRadius))
				neighbors.add(sites[i]);
		}
	...
	public boolean isEnclosingNeighbor(int idCenter, int idToCheck) {
		...
		Vector enc = getEnclosingNeighbors(idCenter);
	...
	public Vector<Integer> getEnclosingNeighbors(int id) {
		...
		Point2d posOfSite = this.sites.get(new Integer(id));
		if (posOfSite == null)
			return new Vector<Integer>();

		Vector<Integer> enclosingNodes = new Vector<Integer>();

		if (sites.get(posOfSite) == null
				|| sites.get(posOfSite).edge_idxlist == null)
			return new Vector<Integer>();

		for (int edge_idx : sites.get(posOfSite).edge_idxlist) {
			...
			int idOfEnclosing = (line.bisectingID[0] == id ? line.bisectingID[1]
					: line.bisectingID[0]);
			enclosingNodes.add(new Integer(idOfEnclosing));
		}
	...
	private boolean overlaps(int id, Point center, int radius) {
		// if (sites.containsKey (new Integer(id)) == false)
		// return false;
		Point2d coord = sites.get(new Integer(id));
		if (coord == null)
			return false;
	...
	protected VonNodeInfo[] getBoundingOrEnclosing(int id, int aoiRadius) {
		...
		VonNodeInfo[] nodeInfos = getAllNodeInfo(boundingOrEnclosing);
	...
	private Integer[] getBoundingOrEnclosingIndices(int id, int aoiRadius) {
		...
		for (int i = 0; i < sites.length; i++) {
			if (isBoundaryNeighborOf(id, sites[i], aoiRadius))
				boundingOrEnclosing.add(sites[i]);
		}
	...
	private Integer[] transFormToIntegerArr(List<VonID> ids) {
	...
	private Integer[] transFormToIntegerArr(VonID[] ids) {
		return transFormToIntegerArr(Arrays.asList(ids));
	}

	public VonNodeInfo[] getAllNodeInfo(Integer[] ids) {
		VonNodeInfo[] nodeInfos = new VonNodeInfo[ids.length];

		for (int i = 0; i < nodeInfos.length; i++) {
			nodeInfos[i] = getNodeInfo(ids[i]);
		}
	...
	public Long[] getAllNodeTimestamps(Integer[] ids) {
		Long[] timestamps = new Long[ids.length];

		for (int i = 0; i < ids.length; i++) {
			if (ids[i].compareTo(this.owningNode.getUniqueValue()) == 0)
				timestamps[i] = Simulator.getCurrentTime();
			else
				timestamps[i] = getTimestamp(ids[i]);
		}
	...
	private boolean insideRegion(int id, Point2d p) {
		...
		boolean b1, b2;
		Point2d siteP = sites.get(new Integer(id));
	...
	private Edge bisect(Site s1, Site s2) {
		...
		// System.out.println ("bisecting (" + s1.coord.x + ", " + s1.coord.y +
	...
	private void out_bisector(Edge e) {
		...
		// System.out.println ("out_bisector [" + (float)e.a + ", " + (float)e.b
		// + ", " + (float)e.c + "]");
		...
		Point2d pt1 = sites.get(new Integer(line.bisectingID[0]));
		Point2d pt2 = sites.get(new Integer(line.bisectingID[1]));
		...
		if (mSites.get(pt1) != null && mSites.get(pt1).edge_idxlist != null) {
			...
			new_edge_idxlist.add(new Integer(e.num));
	...
	private void clip_line(Edge e) {
		...
		// System.out.println ("[" + (float)e.a + ", " + (float)e.b + ", " +
		// (float)e.c + "]");
