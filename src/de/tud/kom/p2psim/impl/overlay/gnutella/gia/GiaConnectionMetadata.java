...
import de.tud.kom.p2psim.impl.overlay.gnutella.gia.GiaQueryManager.EnqueuedQuery;
...
/**
 * Information that is kept by a peer about one of its connected neighbors. Includes the token bucket, the last
 * degree of the neighbor that was observed, the queries that already were relayed to this neighbor, the queue of
 * queries that still need to be relayed to it etc.
 * 
...
public class GiaConnectionMetadata {
	...
	public GiaConnectionMetadata(IGiaConfig config, ILocalClock clock, long initialTAR, int lastDegreeObserved) {
		this.lastDegreeObserved = lastDegreeObserved;
		queryTokens = new TokenBucket(clock, 0, config.getTokenBucketSize(), initialTAR);
		queryUIDsRelayed = new TimeoutSet<Integer>(config.getQueryCacheTimeout());
	}
	...
	/**
	 * Returns the token bucket that regulates the rate of queries that may be sent to the neighbor.
	 * @return
	 */
	...
	/**
	 * Sets the replicated resources of the neighbor to the given set (One-hop replication)
	 * @param resources
	 */
	...
	public String toString() {
		return "Metadata(lastDegreeObserved=" + lastDegreeObserved + ", resources=" + resources  + getQueryTokenBucket() + ", " + "RelayedQIDs=" + queryUIDsRelayed + ")";
	}
	
	/**
	 * Marks the query with the given query UID as relayed, so that it is not relayed to this node again.
	 * @param queryUID
	 */
