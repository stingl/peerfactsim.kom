...
import de.tud.kom.p2psim.impl.overlay.gnutella.common.operations.ConnectOperation;
import de.tud.kom.p2psim.impl.overlay.gnutella.common.operations.PeriodicPingOperation;
...
public abstract class ConnectionManager<TConnectionMetadata, TContact extends GnutellaLikeOverlayContact, TConfig extends IGnutellaConfig, TPong extends GnutellaPong<TContact>> {
	...
	public Map<TContact, Connection> contacts = new HashMap<TContact, Connection>();

	/**
	 * Default constructor
	 * @param owner: the owner node of this connection manager
	 * @param config: the configuration the connection manager is supposed to use
	 * @param size: the maximum connection count the manager may hold
	 * @param pongHdlr: the IPongHandler that handles pong information received by this connection manager
	 * @param foreignAttemptKickProb: the probability that a node is kicked by a foreign connection attempt
	 */
	...
	/**
	 * Sets a pong handler that is used for the construction and evaulation of ping/pong messages.
	 */
	...
	public String toString() {
		 ...
		 getNumberOfContactsInState(ConnectionState.Connected) + "| tot:" +
...
//				+ contacts.size() + " | sem: " + connectSemaphore;
	...
	/**
	 * Notifies the connection manager that the given peers have been discovered.
	 * @param newContacts
	 */
	...
	/**
	 * Puts connection metadata to a given peer. Returns true
	 * if this manager holds a connection to the given peer.
	 * @param c
	 * @param data
	 * @return
	 */
	...
	/**
	 * Returns the metadata for a given contact. Returns null if there is no connection
	 * to the given contact or no contact data was ever stored.
	 * @param c
	 * @return
	 */
	...
	/**
	 * Closes the connecton to the peer c. forContact can be
	 * sent along with the disconnection attempt to suggest the kicked peer
	 * to connect to forContact, too.
	 * 
	...
	/**
	 * Tells the connection manager that an other peer c has triggered a
	 * close attempt.
	 * @param c
	 */
	...
	public Set<TContact> getConnectedContacts() {
		...
		Set<TContact> result = new HashSet<TContact>(
				contacts.size());
	 ...
	 *
	 */
	public class Connection implements IManageableConnection<TContact, TConnectionMetadata> {
		...
		public void startConnectionProcedure() {
			createNewConnectOperation(contact, this, Operations.getEmptyCallback()).scheduleImmediately();
			connectSemaphore--;
		}
		...
		public boolean markAsDead() {
			if (state != ConnectionState.Connected) return false;	//If connection establishment is in progress, do not harm them.
			deadCounter++;
		...
		public void close() {
			...
			if (state == ConnectionState.Connected) connectionEnded(contact, metadata);
		...
		public String toString() {
			return "(" + contact + ", " + state + ", " + deadCounter + ", "
					//+ metadata + ")";
					+ ((getState() != ConnectionState.Connected)? displayTime() + ")":")");
		}
		
		String displayTime() {
			return "t=" + (Simulator.getCurrentTime() - timeEstablished)/1000 + "ms";
		}
		...
		protected void startPing() {
			ping = new PeriodicPingOperation<TContact, TPong>(owner, contact, pongHdlr, this,
		...
		public void connectionTimeouted() {
			if (state != ConnectionState.Pending)		//evtl. hier Fehler!
				connectSemaphore++;
		...
		/**
		 * Returns the failed attempts to contact the peer this connection
		 * is made to. Is reset on every successful attempt.
		 * @return
		 */
		int getDeadCounter() {
			return deadCounter;
	...
	/**
	 * Returns maxSize connected peers randomly, or less if there are less than maxSize.
	 * @param maxSize
	 * @return
	 */
	public List<TContact> getSomeConnectedPeers(int maxSize) {
		return CollectionHelpers.getRandomPartFrom(getConnectedContacts(), maxSize);
	}
	
	/**
	 * Returns maxSize known peers randomly, or less if there are less than maxSize.
	 * @param maxSize
	 * @return
	 */
	...
	/**
	 * Returns the total number of contacts used by this manager. Includes the number of
	 * peers where a connection attempt is currently in progress, not
	 ...
	 * @param l
	 */
	public void addListener(ConnectionManagerListener<TContact, TConnectionMetadata> l) {
		listeners.add(l);
	}
	 ...
	 * @param l
	 */
	public void removeListener(ConnectionManagerListener l) {
		listeners.remove(l);
	}
	 ...
	 */
	protected void lostAConnection(IConnection<TContact, TConnectionMetadata> conn, ConnBreakCause cause) {
		owner.getLocalEventDispatcher().connectionBreak(owner.getOwnContact(), conn.getContact(), cause);
		if (contacts.size() <= 0)
			lostConnectivity();
	}
	...
	public interface ConnectionManagerListener<TContact extends GnutellaLikeOverlayContact, ConnectionMetadata> {
		...
		public void newConnectionEstablished(TContact c, ConnectionMetadata metadata);
	...
	protected Operation createNewConnectOperation(
			TContact contact, IManageableConnection connection,
			OperationCallback<Object> callback) {
		return new ConnectOperation<TContact, TConfig>(owner, contact, this, connection, callback);
	}

	/**
	 * 
	 * Removes all connections. Does not send bye messages in order to notify the connected
	 * peers.
	 * 
	 */
	...
	public Collection<IConnection<TContact, TConnectionMetadata>> getAllConnections() {
		ArrayList<IConnection<TContact, TConnectionMetadata>> result
			= new ArrayList<IConnection<TContact, TConnectionMetadata>>(contacts.size());
		result.addAll(contacts.values());
		return result;
	}
	
	public Collection<IConnection<TContact, TConnectionMetadata>> getEstablishedConnections() {
		Set<IConnection<TContact, TConnectionMetadata>> result = new HashSet<IConnection<TContact, TConnectionMetadata>>();
		
		CollectionHelpers.filter(contacts.values(), result, new Predicate<Connection>() {
			@Override
