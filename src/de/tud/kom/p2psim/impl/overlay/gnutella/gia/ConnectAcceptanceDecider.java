...
/**
 * Decides about accepting, rejecting and dropping contacts for connection according to "Algorithm 1" in the Gia paper.
 * 
...
public class ConnectAcceptanceDecider {
	...
	public ConnectAcceptanceDecider(GiaConnectionManager mgr, GiaOverlayContact requester, int requesterDegree, IGiaConfig config) {
		...
		if (requester.getCapacity() > getMaximumCapacityNeighbor(mgr).getCapacity() || 
				getDegreeOf(highestDegreeNeighbor, mgr) > requesterDegree + config.getConnectDecisionHysteresis()) {
	...
	private int getDegreeOf(GiaOverlayContact c, GiaConnectionManager mgr) {
		return mgr.getMetadata(c).getLastDegreeObserved();
	}
	
	public GiaOverlayContact getMaximumCapacityNeighbor(GiaConnectionManager mgr) {
		int maxCap = -1;
