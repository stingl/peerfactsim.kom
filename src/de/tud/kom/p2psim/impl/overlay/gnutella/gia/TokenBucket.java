...
/**
 * An implementation of a token bucket that can be used for flow control. Lazy filling allows the
 * algorithm to work without any additional operations.
 * 
 * If the tokenAllocationRate is <= 0, no tokens are added to the token bucket at all over time.
 * Else, if the tokenAllocationRate is > 0, it represents the time interval between two tokens
 * that are added to the bucket.
 * 
...
public class TokenBucket {
	...
	public TokenBucket(ILocalClock clock, int initialTokens, int maxTokens, long tokenAllocationRate) {
		this.clock = clock;
	...
	public String toString() {
		return "TBucket(tokens=" + tokens + ", lastAlloc=" + lastTokenAllocated + ", tar=" + tokenAllocationRate + ")";
	}
	...
	public int getTokenCount() {
		if (tokenAllocationRate <= 0) return tokens;
		...
		if ((tokens += addTokens) > maxTokens) tokens = maxTokens;
		return tokens;
	...
	/**
	 * Takes a token from this bucket. Returns false, if the bucket is empty, otherwise
	 * returns true.
	 * @return
	 */
	 ...
	 * Useful if e.g. an application waits for the next token that gets available.
	 * @return
	 */
	public long getTimeToNextTokenAllocation() {
		if (tokenAllocationRate <= 0) return Long.MAX_VALUE;
		getTokenCount();
		long localTime = clock.getCurrentLocalTime();
