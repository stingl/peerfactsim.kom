...
import de.tud.kom.p2psim.impl.util.toolkits.KExtendedLookupMap;
import de.tud.kom.p2psim.impl.util.toolkits.KSortedLookupList;
import de.tud.kom.p2psim.impl.util.toolkits.Predicate;
import de.tud.kom.p2psim.impl.util.toolkits.Comparators.KademliaOverlayContactXORMaxComparator;
 ...
 * {@link LookupCoordinatorClient#getA()}. This list is automatically
 * maintained and {@link #getBestUnqueried()} can be used to determine the next
 * contact from that list that should be queried. {@link #isFinished()}
 * implements a generic termination check that makes use of
 * {@link #isFinishedAppSpecific()} to determine whether the lookup has
 * finished. IsFinished() should be used by subclasses and should not be
 * overwritten, whereas isFinishedAppSpecific() should not be used directly, but
 * is likely to have to be modified to fit into the concrete subclass.
 * 
 * @author Tobias Lauinger <tl1003@rbg.informatik.tu-darmstadt.de>
 * @version 05/06/2011
 */
abstract class AbstractLookupCoordinator<T extends KademliaOverlayID>
	...
	public String toString() {
		return "AbstractLookupCoordinator: " + kClosestNodes;
	}
	...
	public final Set<KademliaOverlayContact<T>> getCurrentlyKnownKClosestNodes() {
		...
		Set<KademliaOverlayContact<T>> result = new java.util.HashSet<KademliaOverlayContact<T>>();
		int i = 0;
		for (KademliaOverlayContact<T> k : kClosestNodes.keySet()) {
			if (kClosestNodes.get(k) == ContactState.QUERIED)
				result.add(k);
			i++;
	 ...
	 *         (false).
	 */
	protected boolean isFinishedAppSpecific() {
	...
	protected boolean containsKQueriedNodes() {
		int i = 0;
		for (ContactState v : kClosestNodes.values()) {
	 ...
	 */
	protected final void insertBestContacts(
			final Collection<KademliaOverlayContact<T>> newContacts) {
		kClosestNodes.putAll(newContacts, ContactState.TO_QUERY, false);
	}
	 ...
	 */
	protected KademliaOverlayContact<T> getBestUnqueried() {
		return kClosestNodes.getMinKey(toQuery);
	}
	...
	private final int getNumOfMessages() {
		int maxConcurrentLookups = ((AbstractKademliaNode<T>) getNode()).getLocalConfig().getMaxConcurrentLookups();
