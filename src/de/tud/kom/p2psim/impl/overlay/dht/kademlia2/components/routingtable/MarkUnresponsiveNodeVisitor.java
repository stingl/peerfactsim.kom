 ...
 * @version 05/06/2011
 */
final class MarkUnresponsiveNodeVisitor<T extends KademliaOverlayID> extends
	 ...
	 */
	protected static final <T extends KademliaOverlayID> MarkUnresponsiveNodeVisitor<T> getMarkUnresponsiveNodeVisitor(
	...
	public final void visit(final LeafNode<T> node) {
		...
		if ((contact = node.kBucket.get(unresponsive)) != null
				...
				&& node.replacementCache.size() > 0) {
			node.kBucket.remove(unresponsive);
			replacement = Collections.max(node.replacementCache.values(),
					this.replacementComparator);
			node.kBucket.put(replacement.getContact().getOverlayID(),
					replacement);
			node.replacementCache.remove(replacement.getContact()
					.getOverlayID());
			if (proximityHandler != null) {
				proximityHandler.contactRemoved(unresponsive);
				proximityHandler.contactAdded(replacement.getContact());
			}
		} else if ((contact = node.replacementCache.get(unresponsive)) != null
				&& (contact.increaseStaleCounter() >= config.getStaleCounter() || node.replacementCache
						.size() == config.getReplacementCacheSize())) {
			node.replacementCache.remove(unresponsive);
		}
	}
