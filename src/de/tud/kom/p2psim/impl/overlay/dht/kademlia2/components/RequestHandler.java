 ...
 * @version 05/06/2011
 */
class RequestHandler<T extends KademliaOverlayID> implements
	 ...
	 */
	protected Set<KademliaOverlayContact<T>> nodeLookupForNode(final KademliaOverlayKey key) {
		int maxReturnedResults = ((AbstractKademliaNode<T>) myNode).getLocalConfig().getMaxReturnedResults();

		return myNode.getKademliaRoutingTable().localLookup(key, maxReturnedResults);
	}
	 ...
	 * @return a Set with K contacts from the local routing table.
	 */
	protected Set<KademliaOverlayContact<T>> nodeLookupForData(final KademliaOverlayKey key) {
		int localBucketSize = ((AbstractKademliaNode<T>) myNode).getLocalConfig().getBucketSize();

		return myNode.getKademliaRoutingTable().localLookup(key, localBucketSize);
	}
	...
	public final void newCloseContactArrived(
		 ...
		 */
		for (final Map.Entry<KademliaOverlayKey, DHTObject> item : allItems.entrySet()) {
			int localBucketSize = ((AbstractKademliaNode<T>) myNode).getLocalConfig().getBucketSize();

			closestAroundKey = myNode.getKademliaRoutingTable().localLookup(item.getKey(), localBucketSize);
