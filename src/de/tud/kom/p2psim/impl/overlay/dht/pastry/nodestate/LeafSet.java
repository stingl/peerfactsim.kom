...
package de.tud.kom.p2psim.impl.overlay.dht.pastry.nodestate;
...
import de.tud.kom.p2psim.impl.overlay.dht.pastry.operations.RequestLeafSetOperation;
...
public class LeafSet implements Iterable<PastryContact> {
	 ...
	 */
	public int getSize() {
		HashSet<PastryContact> l = new HashSet<PastryContact>();
		l.addAll(ccwNodes);
		l.addAll(cwNodes);
	...
	public boolean isInRange(PastryID id) {
		...
		BigInteger ccwDistanceToSmallest = ccwList.getLast().getOverlayID()
				.getCcwDistance(owningNode.getOverlayID());
		BigInteger cwDistanceToLarges = cwList.getLast().getOverlayID()
				.getCwDistance(owningNode.getOverlayID());

		BigInteger ccwDistanceToId = id.getCcwDistance(owningNode
				.getOverlayID());
		BigInteger cwDistanceToId = id.getCwDistance(owningNode.getOverlayID());
