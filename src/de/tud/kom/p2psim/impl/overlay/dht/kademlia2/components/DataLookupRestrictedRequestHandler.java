...
/**
 * A request handler that answers data lookups with a visibility restricted set
 * of nodes from the local routing table. All other requests are answered
 * without restriction.
 * 
 * @author Tobias Lauinger <tl1003@rbg.informatik.tu-darmstadt.de>
 * @version 05/06/2011
 */
public class DataLookupRestrictedRequestHandler<H extends HKademliaOverlayID> extends RequestHandler<H> {
	...
	/**
	 * Constructs a new message handler that restricts results for data lookups.
	 * It has to be manually registered as ProximityListener of
	 ...
	 */
	public DataLookupRestrictedRequestHandler(final TransLayer manager,
			final VisibilityRestrictableNode<H> myNode, ComponentsConfig conf) {
	 ...
	 */
	@Override
	protected final Set<KademliaOverlayContact<H>> nodeLookupForData(
			final KademliaOverlayKey key) {
		int localBucketSize = ((AbstractKademliaNode<H>) myNode)
