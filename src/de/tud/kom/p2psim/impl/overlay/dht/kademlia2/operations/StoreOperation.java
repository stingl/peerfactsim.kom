...
public class StoreOperation<T extends KademliaOverlayID> extends
	 ...
	 */
	protected StoreOperation(final KademliaOverlayKey key,
	 ...
	 */
	private void sendDataItem(final Set<KademliaOverlayContact<T>> receivers) {
		final T myID = getComponent().getTypedOverlayID();
		final KademliaOverlayContact<T> myContact = getComponent()
				.getLocalContact();
		DataMsg<T> message;

		if (!storeForeignData && !receivers.contains(myContact)) {
			// we may store data only if we are part of k closest nodes
			finishOperation(OperationState.ERROR);
			return;
		}

		for (final KademliaOverlayContact<T> receiver : receivers) {
