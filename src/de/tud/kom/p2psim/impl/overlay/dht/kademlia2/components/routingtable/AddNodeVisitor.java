 ...
 * @version 05/06/2011
 */
final class AddNodeVisitor<T extends KademliaOverlayID> extends
	 ...
	 */
	protected static final <T extends KademliaOverlayID> AddNodeVisitor<T> getAddNodeVisitor(
	...
	public final void visit(final LeafNode<T> node) {
		...
		int localBucketSizeOfNode = node.getOwningOverlayNode().getLocalConfig().getBucketSize();
		...
		if ((existingEntryB = node.kBucket.get(newID)) != null) {
		...
		} else if ((existingEntryC = node.replacementCache.get(newID)) != null) {
			...
			} else if (node.replacementCache.size() < config
					.getReplacementCacheSize()) {
				node.replacementCache.put(newID, newEntry);
			} else {
				// drop contact
			}
		}
	 ...
	 */
	protected final boolean isSplittable(final LeafNode<T> node) {
	...
	protected final void buildLeafNode(final LeafNode<T> template,
		 ...
		 */
		final BigInteger prefix = BigIntegerHelpers.shiftLeft(template.getPrefix(), BigInteger.valueOf(suffix), config.getRoutingTreeOrder());
		final LeafNode<T> leaf = new LeafNode<T>(prefix, parent, config, template.getOwningOverlayNode());

		leaf.lastLookup = template.lastLookup;
	...
	protected final RoutingTableEntry<T> getPriorityReplacementCandidate(
		...
		if (replacementStrategy != null && replacementStrategy.compare(newEntry, (bucketMin = Collections.min(node.kBucket.values(), replacementStrategy))) > 0) {
			return bucketMin;
		}
		return null;
	}
	...
	protected final void doPriorityReplacement(final LeafNode<T> node,
		...
		if (node.replacementCache.size() >= config.getReplacementCacheSize()) {
			cacheMin = Collections.min(node.replacementCache.values(),
					replacementStrategy);
			node.replacementCache.remove(cacheMin.getContact().getOverlayID());
		}
		node.replacementCache.put(bucketMin.getContact().getOverlayID(),
				bucketMin);
	}
