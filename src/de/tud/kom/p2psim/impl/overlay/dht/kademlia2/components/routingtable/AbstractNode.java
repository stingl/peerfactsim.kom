 ...
 * @version 05/06/2011
 */
abstract class AbstractNode<T extends KademliaOverlayID> implements Node<T> {
	 ...
	 */
	protected AbstractNode(final BigInteger prefix, final ParentNode<T> parent, final RoutingTableConfig conf, final AbstractKademliaNode<T> owningOverlayNode) {
		this.prefix = prefix;
		this.parent = parent;
		this.config = conf;
	...
	protected AbstractNode(final BigInteger prefix, final ParentNode<T> parent, final RoutingTableConfig conf) {
	 ...
	 */
	protected final ParentNode<T> getParent() {
		return this.parent;
	}
	 ...
	 */
	protected final boolean isResponsibleFor(final T test) {
		// cannot use prefix.bitLength() because that ignores leading zeros
		return prefix.equals(test.getBigInt().shiftRight(config.getIDLength() - getLevel() * config.getRoutingTreeOrder()));
	}
