...
public class FileAnalyzer implements Analyzer {
	...
	private String filePrefix = "outputs/";
	...
	/**
	 * Sets the prefix of the output file (including the path to its directory).
	 * The file name will be constructed by appending the prefix, some
	 * configuration and version information, and the suffix. (If that file
	 * already exists, it will be overwritten. If not, it will be created. If
	 * the path does not exist, it is attempted to create it as well. A new file
	 * is created only if none has been opened by this Analyzer before. If one
	 * file is still open from an earlier Analyzer run, it will be used.)
	 * <p>
	 ...
	 */
	public final void setOutputFilePrefix(final String pathAndPrefix) {
		if (!started) {
			filePrefix = pathAndPrefix;
		}
	}
	...
	/**
	 * Sets the "style" of Kademlia, for example s/k/l/h.
	 * 
	 * @param s
	 *            the style.
	 */
	...
	/**
	 * Sets a description of the current simulation scenario.
	 * 
	 * @param des
	 *            a description, should be filename safe.
	 */
	...
	/**
	 * Sets the seed used in this simulation run.
	 * 
	 * @param sd
	 *            the seed.
	 */
	public final void setSeed(final String sd) {
		if (!started) {
			seed = sd;
		}
	}
