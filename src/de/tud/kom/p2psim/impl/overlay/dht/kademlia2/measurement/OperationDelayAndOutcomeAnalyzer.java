...
import de.tud.kom.p2psim.impl.overlay.dht.kademlia2.operations.AbstractKademliaOperation.OperationState;
...
public class OperationDelayAndOutcomeAnalyzer extends FileAnalyzer implements
	...
	protected final void started() {
		...
		runningAverages = new HashMap<OperationState, Map<Class<? extends Operation<?>>, AvgAccumulator>>();
	 ...
	 */
	private final Map<OperationState, Map<Class<? extends Operation<?>>, Double>> getAvgOperationLatencies() {
		final Map<OperationState, Map<Class<? extends Operation<?>>, Double>> result;
		result = new HashMap<OperationState, Map<Class<? extends Operation<?>>, Double>>(
		...
		for (final Map.Entry<OperationState, Map<Class<? extends Operation<?>>, AvgAccumulator>> outcomeEntry : runningAverages
				.entrySet()) {
			oneOutcomeResult = new HashMap<Class<? extends Operation<?>>, Double>(
					outcomeEntry.getValue().size(), 1.0f);
			result.put(outcomeEntry.getKey(), oneOutcomeResult);
			for (final Map.Entry<Class<? extends Operation<?>>, AvgAccumulator> entry : outcomeEntry
					.getValue().entrySet()) {
	...
	private AvgAccumulator getAvgAcc(final AbstractKademliaOperation op) {
		...
		if (outcomeMap == null) {
			outcomeMap = new HashMap<Class<? extends Operation<?>>, AvgAccumulator>();
	...
	private <C extends Operation<?>> void putAvgAcc(
