...
/**
 * Ermöglicht es, die Quantisierung zu setzen.
 * 
 * Einheit: Refresh-Intervall in realen Millisekunden
 * 
...
public class FPSChanger extends JPanel implements ChangeListener,
	...
	public FPSChanger() {
		...
		this.add(label);
		this.add(slider);
		this.setToolTipText("Quantisierung einstellen (Refresh-Intervall in realen Millisekunden)");
	...
	private double getQuantizationFromSliderValue() {
		...
		return MIN_Q
				+ (MAX_Q - MIN_Q)
				* (1f - Math.pow(1f - ((double) slider.getValue() / SLIDER_UNITS),
						EXPONENT));
	}
