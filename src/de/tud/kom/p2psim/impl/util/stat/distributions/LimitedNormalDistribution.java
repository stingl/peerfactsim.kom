...
public class LimitedNormalDistribution implements Distribution {
	...
	public void writeBackToXML(BackWriter bw) {
		bw.writeSimpleType("mu", conf.getMu());
		bw.writeSimpleType("sigma", conf.getSigma());
		bw.writeSimpleType("min", conf.getMin());
		bw.writeSimpleType("max", conf.getMax());
		bw.writeSimpleType("limitedMin", conf.isLimitedMin());
		bw.writeSimpleType("limitedMax", conf.isLimitedMax());
	}
	...
	public void config(LimitedNormalConfigurer dc) {
		...
		System.out.println("configured Limited Normal Distribution of type "
				+ limitType);
	}

	public double returnValue() {
		double random = pmin + Simulator.getRandom().nextDouble() * pfactor;
		double result;
