...
public abstract class RelationalDB implements XMLSerializable {
	...
	public void readFromXMLFile(File file, SAXParserFactory f) throws SAXException, IOException, ParserConfigurationException {
		DefaultHandler hdlr = getXMLHandler(1);
		f.setNamespaceAware(true);
		SAXParser parser = f.newSAXParser();
		log.info("Loading DB from file " + file);
		long startTime = System.nanoTime();
		parser.parse(file, hdlr);
		long timeDiff = System.nanoTime() - startTime;
		log.info("Loaded DB in " + (timeDiff * 0.000001d) + " milliseconds.");
	}
	
	public void readFromXMLFile(File file) throws SAXException, IOException, ParserConfigurationException {
		SAXParserFactory f = SAXParserFactory.newInstance();
		readFromXMLFile(file, f);
	}
