...
public class UpdateLicense {
	...
	private static String EMAIL = "<peerfact@kom.tu-darmstadt.de>";

	private static String DATE = "05/06/2011";
	...
	private static void insertText(File newFile, File oldFile,
		...
		while ((line = reader.readLine()) != null) {
			...
			else if (line.trim().startsWith("public ") || line.trim().startsWith("class ") || line.trim().startsWith("interface ") || line.trim().startsWith("enum ")) {
				writeHeader(writer);
				writer.write(line);
				writer.newLine();
