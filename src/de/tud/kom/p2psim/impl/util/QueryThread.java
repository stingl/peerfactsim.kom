...
public class QueryThread extends Thread {
	...
	private QueryThread() {
		...
		commands.put("time", new SystemCommand() {
			@Override
			public void execute() {
				System.out.println("Current Time: " + Simulator.getSimulatedRealtime());
			}
		});
		...
		commands.put("events", new SystemCommand() {
			@Override
			public void execute() {
				System.out.println(Simulator.getScheduler().getEventQueueSize() + " events in the queue");
			}
		});
	}
	...
	public void setStopOnExit(boolean stop) {
		if (stop) {
			Simulator.scheduleEvent(null, Simulator.getEndTime(), new SimulationEventHandler() {
