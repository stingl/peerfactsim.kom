 ...
 * @version 05/06/2011
 */
public class SeedChooser extends JPanel implements ActionListener, SeedDeterminationListener, CaretListener, FocusListener {
	...
	public void updateDisplayStrings() {
		...
		String lastUsedSeedText = (det.getLastUsedSeed() != SeedDetermination.NULL_SEED)?" (" + String.valueOf(det.getLastUsedSeed()) + ")" : "";
		...
		fromConfig.setSelected(false);
		customSeed.setSelected(false);
		
		if (det.foundLastUsedSeed()) det.choose(SeedDeterminationChoice.fromLastRun);
			else det.choose(SeedDeterminationChoice.newSeed);
	}

	@Override
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == fromLastRun) det.choose(SeedDeterminationChoice.fromLastRun);
		if (e.getSource() == newSeed) det.choose(SeedDeterminationChoice.newSeed);
		if (e.getSource() == fromConfig) det.choose(SeedDeterminationChoice.fromConfig);
		if (e.getSource() == customSeed) det.choose(SeedDeterminationChoice.customSeed);
	}
